...
I was part of the KaggleX program from Jan-Mar 2023, designed for those who self-identify as being from racial backgrounds that are historically underrepresented in the data science community. As part of this program I was paird with a mentor, [**Mensur Dlakic**](https://www.kaggle.com/tilii7), a Professor of Microbiology at Montana State University and Kaggle Competition Master. 
...
![Process](\img\posts\kagglex\process-flow-v2.png)<br><br>
...
The surrogate ecommerce training data was approximately 3 months long. I used the last month for validation, and first 2 months for training. One thing to note is that to prevent information leakage from the future, we would need to discarded the validation sessions overlapping with the training session. Furthermore, validation data is divided into validation A and validation B, which would contain the ground truths. 

![data-split](\img\posts\kagglex\train_test_split.png)<br><br>
...
<img src="./img/posts/kagglex/covisit.PNG"  width=50% height=50%>
...
The idea behind generating candidates is to reduce the number of `aids` we would have to rank for each test session from 100,000s to just 50-100. <br><br>
...
- **Item features**: Examples include: the count of the `aid` in the last week of the test session, the total and unique count of sessions that an `aid` appears in, the mean and std of an `aid` by hour, dayofweek, or weekend, the distribution of `aid` by `type`, etc. 
...
We then merge all these features with the candidate dataframe. Then, we merge the candidate dataframe with the ground truth using our validation labels, i.e., the validation B dataset. Our data is finally ready for training.<br><br>
...
If we are happy with our local validation score, we repeat this entire pipeline for the full training data. Below were my **test scores**: 
